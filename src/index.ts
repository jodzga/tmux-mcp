...
server.tool(
  "execute-command",
  "Execute a command in the aibot:aiexec tmux pane and wait for it to complete. Returns the command output and exit status. Avoid heredoc syntax (cat << EOF) and other multi-line constructs as they conflict with command wrapping. For file writing, prefer: printf 'content\\n' > file, echo statements, or write to temp files instead. Note: If commands timeout, increase the timeout value - slow shell initialization (especially with kubectl/aws/gcloud completions) can take 10-30 seconds for the first command. Safety: when a shellfirm gate is configured, risky commands are refused and NOT executed; re-invoke with allowRisky:true only after explicit human approval.",
  {
    ...
    allowRisky: z.boolean().optional().describe("Override a shellfirm safety block. Default false: commands flagged as risky are refused. Set true ONLY after a human has explicitly approved the flagged command.")
  },
  async ({ command, timeout, allowRisky }) => {
    try {
      const result = await tmux.executeCommand(command, timeout, allowRisky);
